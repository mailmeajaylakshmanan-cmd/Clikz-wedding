...
export default function QuotationView() {
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      a.href = url;
      a.download = `CLIKZ-Quotation-${quotation.invoiceNo}.pdf`;
  ...
  async function handleWhatsApp() {
    ...
    try {
      const blob = await fetchPDFBlob();
      const file = new File([blob], `CLIKZ-Quotation-${quotation.invoiceNo}.pdf`, { type: 'application/pdf' });
      setShareFile(file);
    } catch (err) {
  ...
  function executeShare() {
    if (navigator.share && navigator.canShare?.({ files: [shareFile] })) {
      navigator.share({
        title: `Quotation ${quotation.invoiceNo} — CLIKZ WEDDING FILMS`,
        files: [shareFile],
      }).catch(e => {
    ...
    <div>
      ...
      <div id="invoice-print" style={doc.wrap}>
        ...
        <div style={doc.footer}>
          ...
          <span>Thank you for considering <strong style={{ color: C.white, fontWeight: 600 }}>CLIKZ WEDDING FILMS</strong>. We look forward to capturing your beautiful moments.</span>
        </div>
      </div>
