  ...
  return (
    <div className="space-y-5 pb-20">
      ...
      <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Invoice Management</h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage bookings and track payments</p>
        </div>
        <Link
          to="/invoices/new"
          className="flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white text-sm px-4 py-2.5 rounded-xl font-semibold transition-all shadow-lg shadow-orange-200 w-full sm:w-auto"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className="bg-white px-4 py-3.5 rounded-xl border border-slate-100 shadow-sm">
        ...
        <table className="w-full text-left min-w-[700px]">
          <thead>
            <tr className="border-b border-slate-100">
              ...
              <th className="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Client / Event</th>
            ...
            {!loading && filtered.map(inv => (
              <tr key={inv._id} className="hover:bg-slate-50/60 transition-colors group">
                ...
                <td className="px-5 py-3.5">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{inv.eventCategoryName || inv.event || '—'}</p>
                </td>
                <td className="px-5 py-3.5 text-sm text-slate-600">{formatDate(inv.eventDate)}</td>
      ...
      <div className="md:hidden space-y-2">
        ...
        {!loading && filtered.map(inv => (
          <div key={inv._id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 space-y-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">{inv.eventCategoryName || inv.event || '—'} · {formatDate(inv.eventDate)}</p>
            </div>
          ...
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-xl overflow-hidden">
            ...
            <div className="px-5 py-4 border-t border-slate-100 flex justify-end">
              <button
                ...
                className="bg-white hover:bg-slate-100 text-slate-700 font-semibold px-4 py-2 rounded-lg border border-slate-200 text-sm transition-colors"
              >
                Close
              </button>
            </div>
